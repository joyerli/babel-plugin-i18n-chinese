...
module.exports = (
  ...
  i18nFunctionModuleId: 'hzero-front/lib/utils/intl',
  isManualI18NImport(path) {
    const { node } = path;
    const moduleId = _.get(node, 'source.value');
    return moduleId === 'utils/intl';
  },
  exclude: [
    '**/webOfficeSdk.js',
    '**/web-office-sdk-v1.1.11.es.js',
  ],
  isI18nFunction(path, { manualI18NImport, autoI18NImport }) {
    ...
    // 如果是否是忽略函数
    if (callee.property.name === 'ignore') {
      if (!callee.object) {
        return false;
      }
      ...
      return callee.object.name === 'intl';
    }
  ...
  generateI18NFunction({
    ...
    const codeText = `oc.${
      (
        filename.includes('@oc') && !filename.includes('@oc-standard-app')
